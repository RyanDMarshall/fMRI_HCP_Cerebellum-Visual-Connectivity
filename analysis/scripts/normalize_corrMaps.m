...
roi_dir = "/projectnb/somerslab/brissend/Wang2015Atlas";
out_dir = sprintf('%s/results/group/corrMaps/%s',rest_dir,proj);

sess_fn = sprintf("%s/session_lists/sessid_rest_nooverlap",rest_dir);
roi_fn = sprintf("%s/roi_names",roi_dir);

sess_fID = fopen(sess_fn,'r');
roi_fID = fopen(roi_fn,'r');
sessions = textscan(sess_fID,'%s'); sessions = sessions{1};
rois = textscan(roi_fID,'%s'); rois = rois{1};
...
for i = 1:length(rois)
    for j = 1:length(sessions)
  	    for hemi = ["lh","rh"]
            sprintf("Normalizing correlation maps for session %s hemi %s ROI %s",sessions{j},hemi,rois{i})
	        data_dir = sprintf("%s/data/%s/corrMaps/%s/%s",rest_dir,sessions{j},proj,hemi);
	        corrMap_fn = sprintf('%s/%s_Wang2015_%s.corrMap.nii',data_dir,hemi,rois{i}); 
    
            corrMap = MRIread(corrMap_fn);
	        corrMap.vol(corrMap.vol <= 0) = nan;
            corrMap.vol = real(fisherz(corrMap.vol));

            corrVols{j} = corrMap.vol;
            corrVols_flat{j} = reshape(corrMap.vol,[],1);
        
            corrVols_flatNorm{j} = (corrVols_flat{j} - mean(corrVols_flat{j},'omitnan'))./std(corrVols_flat{j},'omitnan');
            corrVols_Norm{j} = reshape(corrVols_flatNorm{j}, size(corrVols{1}));
        
            corrNorm_fn = sprintf('%s/%s/Normalized_CorrMaps/%s_%s_r-to-z_normalized.nii',out_dir,hemi,sessions{j},rois{i});
            corrNorm = corrMap; corrNorm.vol = corrVols_Norm{j};
...
%     sPCS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_sPCS_r-to-z_normalized.nii',out_dir,sessions{i});
%     iPCS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_iPCS_r-to-z_normalized.nii',out_dir,sessions{i});
%     midIFS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_midIFS_r-to-z_normalized.nii',out_dir,sessions{i});
%     sPCS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_sPCS_r-to-z_normalized.nii',out_dir,sessions{i});
%     iPCS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_iPCS_r-to-z_normalized.nii',out_dir,sessions{i});
%     midIFS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_midIFS_r-to-z_normalized.nii',out_dir,sessions{i});
% 
%     sPCS_lh = MRIread(sPCS_fn_lh);
%     iPCS_lh = MRIread(iPCS_fn_lh);
%     midIFS_lh = MRIread(midIFS_fn_lh);
%     sPCS_rh = MRIread(sPCS_fn_rh);
%     iPCS_rh = MRIread(iPCS_fn_rh);
%     midIFS_rh = MRIread(midIFS_fn_rh);
% 
%     sPCS_lh_vol{i} = sPCS_lh.vol;
%     iPCS_lh_vol{i} = iPCS_lh.vol;
%     midIFS_lh_vol{i} = midIFS_lh.vol;
%     sPCS_rh_vol{i} = sPCS_rh.vol;
%     iPCS_rh_vol{i} = iPCS_rh.vol;
%     midIFS_rh_vol{i} = midIFS_rh.vol;
% 
%     sPCS_lh_vol_flat{i} = reshape(sPCS_lh_vol{i},[],1);
%     iPCS_lh_vol_flat{i} = reshape(iPCS_lh_vol{i},[],1);
%     midIFS_lh_vol_flat{i} = reshape(midIFS_lh_vol{i},[],1);
%     sPCS_rh_vol_flat{i} = reshape(sPCS_rh_vol{i},[],1);
%     iPCS_rh_vol_flat{i} = reshape(iPCS_rh_vol{i},[],1);
%     midIFS_rh_vol_flat{i} = reshape(midIFS_rh_vol{i},[],1);
% 
...
% 
%     sPCS_vs_2_lh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_lh_sPCS-vs-iPCS+midIFS_norm_corr_diffMap.nii',out_dir,sessions{i});
%     iPCS_vs_2_lh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_lh_iPCS-vs-sPCS+midIFS_norm_corr_diffMap.nii',out_dir,sessions{i});
%     midIFS_vs_2_lh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_lh_midIFS-vs-sPCS+iPCS_norm_corr_diffMap.nii',out_dir,sessions{i});
%     sPCS_vs_2_rh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_rh_sPCS-vs-iPCS+midIFS_norm_corr_diffMap.nii',out_dir,sessions{i});
%     iPCS_vs_2_rh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_rh_iPCS-vs-sPCS+midIFS_norm_corr_diffMap.nii',out_dir,sessions{i});
%     midIFS_vs_2_rh_fn = sprintf('%s/Normalized_CorrMaps/sPCS-iPCS-midIFS/%s_rh_midIFS-vs-sPCS+iPCS_norm_corr_diffMap.nii',out_dir,sessions{i});
% 
...
% 
% %%
% group_sPCS_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_sPCS_norm_corrMap.nii',out_dir);
% group_iPCS_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_iPCS_norm_corrMap.nii',out_dir);
% group_midIFS_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_midIFS_norm_corrMap.nii',out_dir);
% group_sPCS_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_sPCS_norm_corrMap.nii',out_dir);
% group_iPCS_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_iPCS_norm_corrMap.nii',out_dir);
% group_midIFS_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_midIFS_norm_corrMap.nii',out_dir);
% 
...
% 
% group_sPCS_count_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_sPCS_norm_corrMap_count.nii',out_dir);
% group_midIFS_count_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_midIFS_norm_corrMap_count.nii',out_dir);
% group_iPCS_count_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_iPCS_norm_corrMap_count.nii',out_dir);
% group_sPCS_count_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_sPCS_norm_corrMap_count.nii',out_dir);
% group_midIFS_count_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_midIFS_norm_corrMap_count.nii',out_dir);
% group_iPCS_count_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_iPCS_norm_corrMap_count.nii',out_dir);
% 
...
% 
% %%
% group_sPCS_sigcount_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_sPCS_norm_corrMap_sigcount.nii',out_dir);
% group_midIFS_sigcount_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_midIFS_norm_corrMap_sigcount.nii',out_dir);
% group_iPCS_sigcount_lh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_lh_iPCS_norm_corrMap_sigcount.nii',out_dir);
% group_sPCS_sigcount_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_sPCS_norm_corrMap_sigcount.nii',out_dir);
% group_midIFS_sigcount_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_midIFS_norm_corrMap_sigcount.nii',out_dir);
% group_iPCS_sigcount_rh_fn = sprintf('%s/Normalized_CorrMaps/single-condition/ave_rh_iPCS_norm_corrMap_sigcount.nii',out_dir);
% 
