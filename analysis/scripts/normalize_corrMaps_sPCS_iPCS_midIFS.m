...
for i = 1:length(names)
    for j = 1:length(sessions)
  	  	corrMap_fn = sprintf('%s/%s/corrMaps/AV_prob_labels-ctx-to-cb/%s_cb.corrMap.nii',restdir,sessions{j},names(i)); 
        ...
        corrNorm_fn = sprintf('%s/Normalized_CorrMaps/%s_%s_r-to-z_normalized.nii',outdir,sessions{j},names{i});
...
for i=1:length(sessions)
    sPCS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_sPCS_r-to-z_normalized.nii',outdir,sessions{i});
    iPCS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_iPCS_r-to-z_normalized.nii',outdir,sessions{i});
    midIFS_fn_lh = sprintf('%s/Normalized_CorrMaps/%s_lh_midIFS_r-to-z_normalized.nii',outdir,sessions{i});
    sPCS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_sPCS_r-to-z_normalized.nii',outdir,sessions{i});
    iPCS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_iPCS_r-to-z_normalized.nii',outdir,sessions{i});
    midIFS_fn_rh = sprintf('%s/Normalized_CorrMaps/%s_rh_midIFS_r-to-z_normalized.nii',outdir,sessions{i});
