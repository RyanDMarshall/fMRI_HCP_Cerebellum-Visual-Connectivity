...
out_dir=sprintf('%s/normMaps',group_dir);
...
out_dir_pearson_thr99 = sprintf('%s/thr0.99',out_dir_pearson);
out_dir_partial_thr99 = sprintf('%s/thr0.99',out_dir_partial);

%% Make normalized t-statistic maps
outfns=cell(25,1); 
for i=1:length(rois) 
    roi=rois(i); outdir= out_dir;
    outfns{i} = sprintf('%s/%s.lh.pearson.tstat',outdir,roi); 
end
create_norms({pearson_t_lh_full},{pearson_tfce_lh_full},0.99,outfns)

outfns=cell(25,1); 
for i=1:length(rois) 
    roi=rois(i); outdir=out_dir;
    outfns{i} = sprintf('%s/%s.rh.pearson.tstat',outdir,roi); 
end
create_norms({pearson_t_rh_full},{pearson_tfce_rh_full},0.99,outfns)

outfns=cell(25,1); 
for i=1:length(rois) 
    roi=rois(i); outdir=out_dir;
    outfns{i} = sprintf('%s/%s.bilat.pearson.tstat',outdir,roi); 
end
create_norms({pearson_t_bilat_full},{pearson_tfce_bilat_full},0.99,outfns)

outfns=cell(25,1); 
for i=1:length(rois) 
    roi=rois(i); outdir=out_dir;
    outfns{i} = sprintf('%s/%s.lh.partial.tstat',outdir,roi); 
end
create_norms({partial_t_lh_full},{partial_tfce_lh_full},0.99,outfns)

outfns=cell(25,1); 
for i=1:length(rois) 
    roi=rois(i); outdir=out_dir;
    outfns{i} = sprintf('%s/%s.rh.partial.tstat',outdir,roi); 
end
create_norms({partial_t_rh_full},{partial_tfce_rh_full},0.99,outfns)
