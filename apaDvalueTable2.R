...
#' }
#' @export
apa.d.table2 <- function(iv, dv, data, filename=NA, table.number=0, show.conf.interval = TRUE, landscape=TRUE){
  ...
  is_iv <- TRUE #is.valid.name(iv_sub, data_col_names)
  
  dv_sub <- substitute(dv)
  is_dv  <- TRUE #is.valid.name(dv_sub, data_col_names)
