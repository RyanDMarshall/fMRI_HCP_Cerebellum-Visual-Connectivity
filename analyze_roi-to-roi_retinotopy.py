...
def setup_retinotopic_connectivity(which_stat="pearson"):
    ...
    ctx_labels_fn = f"/projectnb/somerslab/ryanmars/projects/CrossTaskAnalysis/proj_allrest/results/group/HCP_SUIT/rois-sorted.txt"
    ctx_labels = np.loadtxt(ctx_labels_fn, dtype=str)
    ...
    n_ctx_rois = np.size(ctx_labels)
    n_cb_rois = np.size(cb_ret_vals)
    n_subs = np.shape(ex_subs_map_fd)[3]
    n_hemis = len(hemis)

    zcorrs_bysub = np.zeros(shape=(n_ctx_rois, n_subs))
    ...
    tstats_bygroup = np.zeros(
        shape=(
            np.shape(ex_group_map)
            + (
                n_ctx_rois,
                n_hemis,
            )
    ...
    for hemi_i in range(0, n_hemis):
        ...
        for roi_i in range(0, n_ctx_rois):
            ...
            zcorrs_bysub[sub_i, roi_i, :] = roi_subs_fisherz_fd
            ...
            tstats_bygroup[:, :, :, roi_i, hemi_i] = roi_group_tstat_fd
        ...
        for cb_roi_i in range(0, n_cb_rois):
            ...
            tstats_bygroup[roi_i, cb_roi_i, hemi_i] = cb_roi_group_map
            mean_tstats_bygroup[roi_i, cb_roi_i, hemi_i] = np.mean(cb_roi_group_map)

            cb_roi_pvals = (
                1 - roi_group_pval_fd[cb_atlas_fd == cb_roi_idx]
            )  # TFCE output is 1 - p value
            cb_roi_zvals = sp.stats.norm.ppf(1 - cb_roi_pvals / 2)
            cb_roi_mean_zval = np.mean(cb_roi_zvals)
            mean_pvals_bygroup[roi_i, cb_roi_i, hemi_i] = 2 * (
                1 - sp.stats.norm.cdf(abs(cb_roi_mean_zval))
            )
        ...
        for sub_i in range(0, n_subs):
            ...
            for roi_j in range(0, n_ctx_rois):
                ...
                corrs_cb_by_roi[roi_i, roi_j, hemi_i] = np.corrcoef(
                    np.ndarray.flatten(roi_group_tstat_fd),
                    np.ndarray.flatten(roi2_group_tstat_fd),
                )[0, 1]
        ...
        output.ctx_labels = ctx_labels
        output.cb_labels = cb_ret_vals
        # output.nvoxels_by_ctx_roi = nvoxels_by_roi_ctx
        # output.nvoxels_by_cb_roi = nvoixels_by_roi_cb
        output.group_tstats_by_ctx_roi = tstats_bygroup
        output.group_mean_tstats = mean_tstats_bygroup
        output.group_mean_pvals = mean_pvals_bygroup
        output.subs_roi_corrs = zcorrs_bysub
        output.subs_mean_zcorr = mean_corrs_bysub
        output.pairwise_roi_corrs = corrs_cb_by_roi
        return output


## Creates one-vs-all connectivity contrasts for each ROI
# input is direct output from setup_retinotopic_connectivity()
# roi can be any ROI from the cortical atlas. If left as default "None", script will iterate over all ROIs
# against can be "all" (one-vs-all maps), "stream" (e.g. FEF vs. remainder of dorsal stream) or "stream-depth" (e.g.
# normalize determines whether the fisherz values should be standardized again before the analysis
# if saveas is None, the function returns the map; otherwise, if saveas is a path to a desired file, the script will
# save the map to that file
# def run_one_vs_all(input, roi=None, against="all", normalize=False, saveas=None):
# 	ctx_labels = input.ctx_labels
# 	roi_group_maps = input.group_tstats_by_ctx_roi
# 	others = np.ones(shape=(np.size(ctx_labels,1)))
# 	if roi is not None:
# 		roi_idx = np.where(np.array(ctx_labels)==roi)[0][0]
# 		if against == "all":
# 			others[rois_idx] = 0
# 		else if against == "stream"
# 			in_stream =


def run_pairwise_ttests(
    proj_dir, ctx_labels, cb_ret_vals, mean_corrs_bysub
):  ## Pairwise cerebellar t-tests by cortical ROI
    output_paired_cb_tstats_fn = f"/{proj_dir}/paired_cb_tstats_by_cortical_roi.csv"
    n_ctx_rois = len(ctx_labels)
    n_cb_rois = len(cb_ret_vals)
    for i in range(0, n_ctx_rois):
        for j, k in list(itertools.combinations(range(0, n_cb_rois), 2)):
            ttest_output = sp.stats.ttest_ind(
                mean_corrs_bysub[:, i, j], mean_corrs_bysub[:, i, k], equal_var=False
            )
        tstat = ttest_output.statistic
        pval = ttest_output.pvalue
        with open(output_paired_cb_tstats_fn, "a") as f:
            print(
                f"{ctx_labels[i]},{cb_ret_vals[j]},{cb_ret_vals[k]},",
                "{:6.4f}".format(tstat),
                ",",
                "{:.{}e}".format(pval, 5),
                file=f,
    ...
    for i in range(0, n_cb_rois):
        for j, k in list(itertools.combinations(range(0, n_ctx_rois), 2)):
            ttest_output = sp.stats.ttest_ind(
                mean_corrs_bysub[:, j, i], mean_corrs_bysub[:, k, i], equal_var=False
            )
            tstat = ttest_output.statistic
            pval = ttest_output.pvalue
            with open(output_paired_ctx_tstats_fn, "a") as f:
                print(
                    f"{cb_ret_vals[i]},{ctx_labels[j]},{ctx_labels[k]},",
                    "{:6.4f}".format(tstat),
                    ",",
                    "{:.{}e}".format(pval, 5),
                    file=f,
...
def plot_cb_dendrogram(input):
    ...
    dist = 1 - corrs_cb_by_roi[:, :, i]
    links = linkage(dist, method="ward")
    dendrogram(links, ax=ax, labels=ctx_labels, leaf_rotation=45.0)
    plt.xlabel("ROIs")
    plt.ylabel("Distance")
...
def plot_rose(
    ...
    plot_which_structure = [
        "ctx" if elem in ctx_labels else "cb" if elem in cb_label_order else elem
        for elem in plot_which_labels
    ]

    if n_hemis == 2:
        connectivity_ctx_lh = connectivity[:, :, 0]
        connectivity_ctx_rh = connectivity[:, :, 1]
        connectivity_ctx_lh = pd.DataFrame(
            connectivity_ctx_lh, index=ctx_labels, columns=cb_labels
        )
        connectivity_ctx_rh = pd.DataFrame(
            connectivity_ctx_rh, index=ctx_labels, columns=cb_labels
        )
        connectivity_ctx_lh = connectivity_ctx_lh.loc[
            ctx_label_order, cb_label_order_full
        ]
        connectivity_ctx_rh = connectivity_ctx_rh.loc[
            ctx_label_order, cb_label_order_full
        ]
        ...
        connectivity = pd.DataFrame(
            connectivity, index=ctx_label_order, columns=cb_labels
        )
        connectivity = connectivity.loc[ctx_label_order, cb_label_order_full]

    fig, axes = plt.subplots(
        nrows=n_rows, ncols=n_cols, subplot_kw={"projection": "polar"}
    )
    ...
    for i in range(0, len(plot_which_labels)):
        for j in range(0, n_hemis):
            ...
            if (
                structure == "ctx"
            ):  # For a cortical ROI we plot against cerebellar ROIs and vice versa
                angles = np.array(
                    np.linspace(
                        0, 2 * np.pi, len(cb_label_order), endpoint=False
                    ).tolist()
                )
                label_order = cb_label_order
                ctx_idx = [
                    index for index, item in enumerate(ctx_label_order) if item == roi
                ]

                left_idx = [
                    index
                    for index, item in enumerate(cb_label_order_full)
                ...
                angles = np.array(
                    np.linspace(
                        0, 2 * np.pi, len(ctx_label_order), endpoint=False
                    ).tolist()
                )
                label_order = ctx_label_order
                cb_idx_lh = np.where(
                    [
                        elem.endswith(roi) and elem.startswith("left_")
                        for elem in cb_label_order_full
                    ]
                )
                cb_idx_rh = np.where(
                    [
                        elem.endswith(roi) and elem.startswith("right_")
                        for elem in cb_label_order_full
                    ]
                )

                connectivity_lh = np.ndarray.flatten(
                    connectivity[25 * j : 25 * (j + 1), cb_idx_lh]
                )
                connectivity_rh = np.ndarray.flatten(
                    connectivity[25 * j : 25 * (j + 1), cb_idx_rh]
                )
            ...
            if plot_which_stat == "p":
                ax.set_yticks(
                    [0.0, 1.0, 2.0, 3.0, 4.0]
                )  # Customize the y-axis ticks as needed
